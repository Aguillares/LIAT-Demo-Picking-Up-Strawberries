...
class Robot(Node):
    ...
    def callback(self, msg):
        '''
        self.bot.arm.set_single_joint_position(joint_name='waist', position=math.radians(90),moving_time=1,accel_time = 0.02)
        
        #print("Waist "+str(mat))
        self.bot.arm.set_ee_pose_components(z=self.fixedZ,x = 0,y =0, moving_time=2)
        mat=self.bot.arm.get_ee_pose_command()
        
        mat [2,3] = 
        print("Z "+str(self.bot.arm.get_ee_pose_command))
        sleep(5)
        '''
        ...
        if len(info[0]) == 0 or len(redInfo) == 0:
            ...
            '''
            if info[0,1].astype(int)>self.limSupY:
                self.deltaStrawZ = 0.001
            elif info[0,1].astype(int)<self.limInfY:
                self.deltaStrawZ = -0.001
            else:
                self.deltaStrawZ = 0
                
            self.deltaZ = self.deltaZ +self.deltaStrawZ
            
            self.bot.arm.set_ee_pose_components(z=self.fixedZ, x = self.deltaX, y=self.deltaY, moving_time=2)
            '''
    ...
    def initialPosition(self):
        #self.bot.arm.go_to_home_pose(moving_time = 1,accel_time = 0.02)
        self.bot.arm.go_to_sleep_pose(moving_time = 3.0,accel_time = 0.02)
        
        #self.bot.arm.go_to_sleep_pose(moving_time = 3.0,accel_time = 0.02)
        #self.bot.arm.set_single_joint_position(joint_name='shoulder', position=math.radians(-78),moving_time=1,accel_time = 0.02)
        
        self.bot.arm.set_ee_pose_components(z=self.fixedZ)
        
        #self.bot.arm.set_ee_pose_components(z=self.fixedZ+0.5,x=0.2)
        #self.bot.arm.set_ee_pose_components(z=self.fixedZ,x=0.5)
        #self.bot.arm.set_ee_pose_components(z=self.fixedZ,x=-0.2)
        print("Angle = "+str(self.angleJoint))
        #self.bot.arm.set_single_joint_position(joint_name='waist', position=math.radians(self.angleJoint),moving_time=1,accel_time = 0.02)
        
        '''
        mat=self.bot.arm.get_ee_pose_command()
        
        mat [2,3] = self.fixedZ-0.1
        
        theta_list,solved=self.bot.arm.set_ee_pose_matrix(mat)
        print(str(solved), str(theta_list))
        '''
        #self.bot.arm.set_ee_cartesian_trajectory(z=0.1,moving_time=2.0)
       # self.bot.arm.set_single_joint_position(joint_name='waist', position=math.radians(self.angle),moving_time=0.2,accel_time = 0.02)
      #self.deltaX = math.sin(math.radians(self.angle))*self.constant
       # self.bot.arm.set_ee_pose_components(x=self.deltaX,z=self.deltaZ,moving_time=3.0)
